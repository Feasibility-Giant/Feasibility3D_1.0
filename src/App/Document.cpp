...
static std::string checkFileName(const char *file) {
    ...
    // that the user won't accidentally overwrite other files that may be critical.
    if (!ext || (!boost::iequals(ext+1, "fcstd") && !boost::iequals(ext+1, "f3d"))) {
        if (ext && ext[1] == 0) {
            // Handle case where there's a dot but no extension
            fn += "FCStd";
        } else {
            // Check a user setting or some condition to decide the extension
            if (App::GetApplication().GetParameterGroupByPath
                    ("User parameter:BaseApp/Preferences/Document")->GetBool("UseF3DFormat", false)) {
                fn += ".f3d";
            } else {
                fn += ".FCStd";
            }
