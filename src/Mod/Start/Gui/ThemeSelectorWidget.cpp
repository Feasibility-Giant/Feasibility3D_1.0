...
void ThemeSelectorWidget::setupButtons(QBoxLayout* layout)
{
    ...
    std::map<Theme, QString> themeMap {{Theme::Classic, tr("Feasibility3D Classic")},
                                       {Theme::Dark, tr("Feasibility3D Dark")},
                                       {Theme::Light, tr("Feasibility3D Light")}};
    ...
    for (const auto& theme : themeMap) {
        ...
        if (theme.first == Theme::Classic && styleSheetName.isEmpty()) {
            button->setChecked(true);
                 ...
                 && styleSheetName.contains(QLatin1String("Feasibility3D Light"),
                                            Qt::CaseSensitivity::CaseInsensitive)) {
            button->setChecked(true);
        }
        else if (theme.first == Theme::Dark
                 && styleSheetName.contains(QLatin1String("Feasibility3D Dark"),
                                            Qt::CaseSensitivity::CaseInsensitive)) {
            button->setChecked(true);
        }
...
void ThemeSelectorWidget::themeChanged(Theme newTheme)
{
    ...
    switch (newTheme) {
        case Theme::Classic:
            prefPackManager->apply("Feasibility3D Classic");
            break;
        case Theme::Dark:
            prefPackManager->apply("Feasibility3D Dark");
            break;
        case Theme::Light:
            prefPackManager->apply("Feasibility3D Light");
            break;
    }
...
void ThemeSelectorWidget::retranslateUi()
{
    ...
    _buttons[static_cast<int>(Theme::Dark)]->setText(tr("Feasibility3D Dark", "Visual theme name"));
    _buttons[static_cast<int>(Theme::Light)]->setText(tr("Feasibility3D Light", "Visual theme name"));
    _buttons[static_cast<int>(Theme::Classic)]->setText(tr("Feasibility3D Classic", "Visual theme name"));
}
