...
{
    sGroup        = "Help";
    sMenuText     = QT_TR_NOOP("Feasibility3d website");
    sToolTipText  = QT_TR_NOOP("The Feasibility3d website");
    sWhatsThis    = "Std_FreeCADWebsite";
    sStatusTip    = QT_TR_NOOP("Feasibility3d website");
    sPixmap       = "internet-web-browser";
    eType         = 0;
}
...
void StdCmdFreeCADWebsite::activated(int iMsg)
{
    ...
    std::string defaulturl = "https://feasibility-giant.com";
    
    // Retrieve the parameter group for websites
    ParameterGrp::handle hURLGrp = App::GetApplication().GetParameterGroupByPath("User parameter:BaseApp/Preferences/Websites");
    
    // Get the URL from the parameter group, defaulting to 'defaulturl' if none is set
    std::string url = hURLGrp->GetASCII("WebPage", defaulturl.c_str());
    
    // Ensure that 'https://' is not prepended multiple times
    if (url.find("https://") != 0) {
        url = "https://" + url;
    }

    // Save the (corrected) URL back to the parameter group
  ...
  :Command("Std_FreeCADUserHub")
{
    ...
    sToolTipText  = QT_TR_NOOP("Documentation for users on the Feasibility3d website");
  ...
  :Command("Std_FreeCADPowerUserHub")
{
    ...
    sToolTipText  = QT_TR_NOOP("Python scripting documentation on the Feasibility3d website");
...
{
    sGroup        = "Help";
    sMenuText     = QT_TR_NOOP("Feasibility3D Forum");
...
{
    sGroup        = "Help";
    sMenuText     = QT_TR_NOOP("Feasibility3D FAQ");
    sToolTipText  = QT_TR_NOOP("Frequently Asked Questions on the Feasibility3d website");
