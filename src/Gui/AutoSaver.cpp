            ...
            && !doc->testStatus(App::Document::TempDoc))
    {
        ...
        // open extra scope to close ZipWriter properly
        {
            ...
            // only create the file if something has changed
                ...
                // Extract the file extension from the document's file name
                std::string fileName = doc->getFileName();  // Assumes getFileName() exists
                std::string fileExtension = Base::FileInfo(fileName).extension();

                // Determine the appropriate file extension for the recovery file
                if (boost::iequals(fileExtension, "f3d")) {
                    fn += "/fc_recovery_file.f3d";
                } else {
